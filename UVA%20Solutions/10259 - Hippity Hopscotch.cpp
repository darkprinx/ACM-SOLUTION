...
int solv(int x,int y)
{
    ...
    for(int i=1;i<=k;i++)
        if(x-i>=0&&ara[x-i][y]>ara[x][y])
        a=max(a,solv(x-i,y)+ara[x-i][y]);
    for(int i=1;i<=k;i++)
        if(y-i>=0&&ara[x][y-i]>ara[x][y])
        b=max(b,solv(x,y-i)+ara[x][y-i]);
    for(int i=1;i<=k;i++)
        if(x+i<n&&ara[x+i][y]>ara[x][y])
        c=max(c,solv(x+i,y)+ara[x+i][y]);
    for(int i=1;i<=k;i++)
        if(y+i<n&&ara[x][y+i]>ara[x][y])
        d=max(d,solv(x,y+i)+ara[x][y+i]);

    return dp[x][y]=max(a,max3(b,c,d));
