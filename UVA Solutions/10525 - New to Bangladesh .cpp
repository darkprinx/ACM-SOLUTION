...
void floyed()
{
    ...
    for(int j=0;j<=n;j++)
    {
        if(ara[i][j]>ara[i][k]+ara[k][j])
        {
            ...
            mara[i][j]=mara[i][k]+mara[k][j];
        }
        else if(ara[i][j]==ara[i][k]+ara[k][j])
        {
            if(mara[i][j]>mara[i][k]+mara[k][j])
            mara[i][j]=mara[i][k]+mara[k][j];
        }
    }
}
...
int main()
{
    ...
    while(tt--)
    {
        ...
        loop(i,0,m)
        {
            ...
            fs(d);
            if(ara[a][b]==c)
               mara[a][b]=mara[b][a]=min(mara[a][b],d);
            else if(ara[a][b]>c)
            {
                ara[a][b]=ara[b][a]=c;
                mara[a][b]=mara[b][a]=d;
        ...
        loop(i,0,x)
        {
            ...
            fs(b);
            if(ara[a][b]!=inf)
                pf("Distance and time to reach destination is %lld & %lld.\n",mara[a][b],ara[a][b]);
            else
                pf("No Path.\n");
        }
