...
bool cmp(st x,st y)
{
    if(x.a==y.a||x.a==y.b||x.a==y.c)
        return false;
    if(x.b==y.a||x.b==y.b||x.b==y.c)
        return false;
    if(x.c==y.a||x.c==y.b||x.c==y.c)
        return false;
    return true;
}
...
int cs=0;
    while(fs(n)&&n)
    {
        ...
        for(int i=0;i+2<n;i++)
            ...
            for(int k=j+1;k<n;k++)
            if(cmp(ara[i],ara[j])&&cmp(ara[j],ara[k])&&cmp(ara[i],ara[k]))
                mn=max(mn,ara[i].s+ara[j].s+ara[k].s);
        pf("Case %d: %d\n",++cs,mn);
    }
    return 0;
}
