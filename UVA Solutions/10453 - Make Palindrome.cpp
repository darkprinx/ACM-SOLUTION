/**
   ...
   Problem Name: 10453 - Make Palindrome
       Platform: UVa
      Algorithm: DP
...
int solv(int i,int j)
{
    ...
    if(ara[i]==ara[j])
        ...
        sum = min(solv(i+1,j),solv(i,j-1))+1;
    return dp[i][j] = sum;
}


void dp_pr(int i,int j)
{
    if(i>j)
        return;
    ...
    else if(solv(i,j)==solv(i,j-1)+1)
    {
        ans.pb(ara[j]);
        dp_pr(i,j-1);
        ans.pb(ara[j]);
    }
    else if(solv(i,j)==solv(i+1,j)+1)
    {
        ans.pb(ara[i]);
        dp_pr(i+1,j);
        ans.pb(ara[i]);
