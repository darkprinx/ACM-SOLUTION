...
//freopen("out.txt","w",stdout);
     ...
     while(scanf("%d%d%d%d%d%d",&p,&r,&bh,&of,&yh,&m)==6)
    {
        ...
        for(int i=0; i<=p; i++)
            if(ara[bh][of]==ara[bh][i]+ara[i][of])
                mark[i]=false;
            ...
            if(mark[k])
                for(int i=0; i<=p; i++)
                    if(mark[i])
                        for(int j=0; j<=p; j++)
                            if(mark[j])
                                mara[i][j]=min(mara[i][j],mara[i][k]+mara[k][j]);

        if(mara[yh][m]==xxx||!mark[yh]||!mark[m])
            pf("MISSION IMPOSSIBLE.\n");
        else
            pf("%d\n",mara[yh][m]);
