/*
   ...
   Problem Name: 10690 - Expression Again
       Platform: Uva
      Algorithm: DP
...
int ara[102];
pii dp[102][52][5104];
bool mark[102][52][5104];
int sum;
int xx=2502;

pii solv(int i,int j,int cnt)
{
    if(j<0)
        return pii((sum-cnt)*cnt,(sum-cnt)*cnt);
    if(i<0)
        return pii(-inf,inf);

    if(mark[i][j][cnt+xx])
        return dp[i][j][cnt+xx];


    pii p1(-inf,inf),p2(-inf,inf);

    p1=solv(i-1,j-1,cnt+ara[i]);
    p2=solv(i-1,j,cnt);
    mark[i][j][cnt+xx]=true;
    return dp[i][j][cnt+xx]=pii(max(p1.F,p2.F),min(p1.S,p2.S));
}
