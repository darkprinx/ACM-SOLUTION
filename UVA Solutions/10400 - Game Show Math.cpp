...
int solv(int i,int tot)
{
    ...
    int ans = 0;
    ans|=solv(i+1,tot+ara[i]);
    ans|=solv(i+1,tot-ara[i]);
    ans|=solv(i+1,tot*ara[i]);
    if(tot%ara[i]==0)
        ans|=solv(i+1,tot/ara[i]);
    return dp[i][tot+x] = ans;
}

void dp_pr(int i,int tot)
{
    ...
    else if(abs(mc)<=x&&dp[i+1][mc+x]==1)
    {
        if(i)
            cout<<'-';
        cout<<ara[i];
    ...
    else if(abs(dc)<=x&&dp[i+1][dc+x]==1)
    {
        if(i)
            cout<<'*';
        cout<<ara[i];
    ...
    else if(rc!=-1&&abs(rc)<=x&&dp[i+1][rc+x]==1)
    {
        if(i)
            cout<<'/';
        cout<<ara[i];
