...
{
    string nd,cst,par;
    st(string xx,string yy,string zz):
        nd(xx),cst(yy),par(zz){};
    bool operator <(const st &op)const
    {
        return cst.size()>op.cst.size();
    }
};

int n;
string s,des;
map<string , vector<string> > nod,cost;
...
string bfs()
{
    ...
    while(!q.empty())
    {
        ...
        if(u.nd==des)
            return u.cst;
        for(int i=0;i<nod[u.nd].size();i++)
        {
            string v=nod[u.nd][i];
            string cv=cost[u.nd][i];

            if(cv[0]!=u.cst[0]&&!mark[cv]&&v!=u.par)
            {
                q.push(st(v,cv+u.cst,u.nd));
                mark[cv]=true;
            }
        }
    }
