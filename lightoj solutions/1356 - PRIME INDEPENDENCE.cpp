/*
   ...
   Problem Name: 1356 - PRIME INDEPENDENCE
       Platform: LightOj
      Algorithm: Hopocroft Karp BPM
...
int main()
{
    ...
    for(int kk=1; kk<=tt; kk++)
    {
        ...
        for(int i=1; i<=n; i++)
            ...
            {
                tmp=ara[i]/pfact[i][j];
                if(pos[ara[i]][0]!=-1&&pos[tmp][1]!=-1)
                    nod[pos[tmp][1]].pb(i);
                else if(pos[ara[i]][1]!=-1&&pos[tmp][0]!=-1)
                    nod[i].pb(pos[tmp][0]);
            }
        pf("Case %d: %d\n",kk,n-hopcop());
