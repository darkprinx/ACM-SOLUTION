...
void dfs(int i,vector<ll> &a,vector<ll> &b,ll sum)
{
    a.pb(sum);
    if(i==b.size())
        return;
    dfs(i+1,a,b,sum);
    dfs(i+1,a,b,sum+b[i]);
    dfs(i+1,a,b,sum+b[i]*2);
}

int main()
{

    int test,n,lf,rf,val;
    fs(test);

    for(int k=1;k<=test;k++)
    {
        vector<ll> a,b,ara,mara;
        ll xx;
        ...
        for(int i=0; i<mara.size(); i++)
            if(binary_search(all(ara),val-mara[i]))
                flag=true;
