...
int dp[1005][105],n,m,cap;
int ara[1006];
vi nod[1005],cost[1005];
...
void bfs(int s,int des)
{
    ...
    while(!q.empty())
    {
        ...
        if(u.f<cap)
            if(dp[u.u][u.f+1]>dp[u.u][u.f]+ara[u.u])
            {
                dp[u.u][u.f+1]=dp[u.u][u.f]+ara[u.u];
                q.push(st(u.u,u.f+1,dp[u.u][u.f+1]));
            }

        for(int i=0; i<nod[u.u].size(); i++)
        {
            int v=nod[u.u][i];

            if(cost[u.u][i]<=u.f)
                if(dp[v][u.f-cost[u.u][i]]>u.cst)
                {
                    dp[v][u.f-cost[u.u][i]]=u.cst;
                    q.push(st(v,u.f-cost[u.u][i],u.cst));
                }
        }
    }
    pf("impossible\n");
