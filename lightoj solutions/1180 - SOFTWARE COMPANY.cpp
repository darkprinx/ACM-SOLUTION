/**
   ...
   Problem Name: 1180 - SOFTWARE COMPANY
       Platform: LightOJ
      Algorithm: Binary Search + DP
...
int solv(int id,int p1,int p2)
{
    ...
    for(int i=0;i<=p1&&i*ara[id].F<=mid;i++)
    {
        int rm=(mid-(i*ara[id].F))/ara[id].S;
        if(rm>p2)
            rm=p2;
        ans|=solv(id+1,p1-i,p2-rm);
    }
    return dp[id][p1][p2]=ans;
}
