/*
   ...
   Problem Name: 1298 - ONE THEOREM, ONE YEAR
       Platform: LightOj
     Algorithom: DP+Number Theory
  Author's Name: RC Tushar

  COMMENT: ONE OF MY FAVORITE PROBLEMS
...
void prec()
{
    ...
    {
        ara[i][i]=1;
        dp[i][i]=((dp[i-1][i-1]%MOD)*(prm[i]-1))%MOD;
    }

    for(int j=1;j<=500;j++)
    for(int i=j+1;i<=500;i++)
    {
        ara[i][j]=(((ara[i-1][j]%MOD)*prm[j])%MOD+ara[i-1][j-1])%MOD;
        dp[i][j]=(dp[j][j]%MOD*ara[i][j]%MOD)%MOD;
    }
}
