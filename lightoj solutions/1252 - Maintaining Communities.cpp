/*
   ...
   Problem Name: MAINTAINING COMMUNITIES
       Platform: LightOj
      Algorithm: Sibling DP
...
void solv(int u,int par)
{
    for(int i=0;i<nod[u].size();i++)
    {
        ...
        for(int j=k;j>=0;j--)
            {
                dp[u][j]+=mx+1;
                for(int lim=0;lim<=j-ara[u][v];lim++)
                dp[u][j]=min(dp[u][j],dp[u][j-ara[u][v]-lim]+dp[v][lim]);
            }
    }
}
