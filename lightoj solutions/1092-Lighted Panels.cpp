...
int solv(int id,int cur,int pre)
{
    if(id==r)
    {
        if(pre==(1<<c)-1)
    ...
    for(int i=0;i<(1<<c);i++)
    {
        ...
        if(id==0||(tmp[0]==(1<<c)-1))
            ans=min(ans,cnt+solv(id+1,tmp[2],tmp[1]));
    }
    return dp[id][cur][pre]=ans;
}
