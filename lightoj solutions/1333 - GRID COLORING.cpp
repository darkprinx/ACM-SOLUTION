/*
   ...
   Problem Name: 1333 - GRID COLORING
       Platform: LightOj
...
pii ara[505];
ll mud=1000000000;
bool chk(int x,int y,int q)
{
    pii xx=pii(x,y);
    for(int i=0;i<q&&ara[i].F<=x;i++)
    if(ara[i]==xx)
    return true;
    return false;
}
...
int main()
{
    ...
    for(int kk=1;kk<=tt;kk++)
    {
        ...
        pf("Case %d: %lld\n",kk,(binPow(k,aa,mud)*binPow(k-1,bb,mud))%mud);
    }
    return 0;
}
