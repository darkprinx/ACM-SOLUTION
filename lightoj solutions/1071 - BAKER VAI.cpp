...
int n,m;
int solv(int r,int c1,int c2,int flag)
{
    if(dp[r][c1][c2][flag]!=-1)
        return dp[r][c1][c2][flag];

    int sum=0;
    if(!flag)
    {
        if(c2+1<m)
        sum=max(sum,solv(r,c1,c2+1,0)+ara[r][c2+1]);
        if(c1<c2)
        sum=max(sum,solv(r,c1,c2,2));
    }
    else if(flag==1)
    {
        if(c1+1<c2)
        sum=max(sum,solv(r,c1+1,c2,1)+ara[r][c1+1]);
        sum=max(sum,solv(r,c1,c2,0));
    }
    else if(flag==2&&r+1<n)
        sum=max(sum,solv(r+1,c1,c2,1)+ara[r+1][c1]+ara[r+1][c2]);
    return dp[r][c1][c2][flag]=sum;
}
