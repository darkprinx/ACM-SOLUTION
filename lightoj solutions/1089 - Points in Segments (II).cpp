/*
   ...
   Problem Name: 1089 - Points in Segments (II)
       Platform: LightOj
     Algorithom: Segment tree + array compreshion (such a pathetic problem cost me 7 RE)
...
int tree[3*M],x,y,lazy[3*M],qu[M],mara[M];

void update(int nod,int b,int e)
{
    if(x>mara[e]||y<mara[b])
        return ;
    if(mara[b]>=x&&mara[e]<=y)
    {
        tree[nod]+=(e-b+1);
        lazy[nod]++;
        return;
